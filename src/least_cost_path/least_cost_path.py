...
def least_cost_path_analysis(
    ...
    try:
        ...
        ds3857 = gdal.Open(dem_reprojected)
        ...
        gdal.Warp(
            ...
            srcDSOrSrcDSTab=dem_reprojected,
            xRes=orig_xres * 4,
            yRes=orig_yres * 4,
            resampleAlg="average",
            format="GTiff",
        ...
        g, gt, n_rows, n_cols = build_cost_graph(
            dem_pooled, water_raster_layer.source()
        )

        terminal_nodes_set = set()
        ds_water = gdal.Open(water_raster_layer.source())
...
def least_cost_path_analysis_tool(
        points_layer: QgsVectorLayer,
        dem_layer: QgsRasterLayer,
        water_raster_layer: QgsRasterLayer,
        feedback = None
...
def prepare_cost_layer(project_folder: str, dem_input_path: str) -> QgsRasterLayer:
    """
    Создает и возвращает слой стоимости (cost layer) из DEM:
    1. Перепроецирует в EPSG:3857
    2. Понижает разрешение в 4 раза (усреднение)
    
    Args:
        project_folder: Путь к папке проекта
        dem_input_path: Путь к исходному DEM файлу
        
    Returns:
        QgsRasterLayer: Готовый слой стоимости или None при ошибке
    """
    try:
        # 1. Перепроецирование в Web Mercator
        dem_3857_path = f"{project_folder}/srtm_output_3857.tif"
        gdal.Warp(
            dem_3857_path,
            dem_input_path,
            dstSRS="EPSG:3857",
        ...
        # 2. Понижение разрешения
        dem_pooled_path = f"{project_folder}/srtm_output_3857_pooled.tif"
        ds = gdal.Open(dem_3857_path)
        gt = ds.GetGeoTransform()
        
        gdal.Warp(
            destNameOrDestDS=dem_pooled_path,
            srcDSOrSrcDSTab=dem_3857_path,
            xRes=gt[1] * 4,
            yRes=abs(gt[5]) * 4,
            resampleAlg="average",
            format="GTiff",
        ...
        # 3. Создаем и возвращаем слой
        cost_layer = QgsRasterLayer(dem_pooled_path, "DEM Cost Layer")
        return cost_layer if cost_layer.isValid() else None

    except Exception as e:
        QMessageBox.critical(None, "Ошибка подготовки DEM", str(e))
        return None
